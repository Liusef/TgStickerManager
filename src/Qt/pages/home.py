...
class PackWidget(ClickWidget):
    def __init__(self, pack: TgStickerPack):
        ...
        text = gui.basic_label(pack.name)
...
class PackGridView(QWidget):
    ...
    def loading_widget(self) -> QWidget:
        layout = QVBoxLayout()
        layout.addWidget(gui.basic_label("Loading...", gui.generate_font(QLabel(), 12)))
    ...
    async def show_info(self):
        ...
        if len(sns) == 0:
            ...
            self.layout().addWidget(gui.basic_label("You don't have any Sticker Packs\n"
                                         "Press Refresh to sync your packs from Telegram or Add to create a new pack!",
